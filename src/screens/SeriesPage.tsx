...
import { Tv, Filter, X, Sparkles } from "lucide-react";
...
export default function SeriesPage() {
  const { series, categories, loading } = useData();
  ...
  const genres = useMemo(() => {
    ...
    series.forEach((s: Series) => s.genres?.forEach((g) => set.add(g)));
    return Array.from(set).sort();
  }, [series]);

  const years = useMemo(() => {
    const set = new Set<number>();
    series.forEach((s: Series) => set.add(s.year));
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [series, category, genre, year, sort]);
  ...
  return (
    <div className="pb-12">
      <PageHeader
        title="TV Series"
        subtitle={`Browse our collection of ${series.length} series`}
      ...
      <div className="px-4 sm:px-6 lg:px-12 mb-3 flex items-center gap-2 text-sm text-muted-foreground">
        ...
        Showing <span className="text-foreground font-semibold">{filtered.length}</span> serie{filtered.length !== 1 ? "s" : ""}
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center text-center py-20 px-4">
          ...
          <h3 className="text-xl font-bold text-foreground mb-2">No series match your filters</h3>
